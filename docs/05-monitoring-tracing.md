...
1. Review the high-level cards (for example: **Running agents**, **Estimated cost**, **Agent success rate**, **Token usage**) only as optional context.

    > **Important (UI drift)**: Foundry has moved to an agent-focused dashboard. Older, fine-grained cards like **total token count / prompt token count / completion token count / total requests** may not be available in this view.

    > **Important (data pipeline mismatch)**: This lab uses direct `chat.completions.create()` calls with OpenTelemetry in `src/tests/run_monitoring.py`, not Foundry Agent runtime calls. Because of that, Foundry agent dashboard cards can show `No data to show` even when telemetry is correctly flowing to Application Insights.

### Why step-10 may fail (`ui_or_flow_drift_blocking`)

If your instructions expected token breakdown cards in **Operate > Overview**, the failure is usually a combination of:

1. **Flow mismatch**: Monitoring traffic is generated by direct OpenAI chat completions + OpenTelemetry, which does not populate Foundry Agent-specific metrics.
1. **UI redesign**: Foundry no longer exposes the old fine-grained token breakdown cards used by earlier lab versions.

For this lab, treat Application Insights trace/log data as the source of truth.

### Review token usage across versions

Use `python src/tests/check_traces.py` output as your primary comparison surface.

For each test span (for example, `v3_trail-difficulty`), compare:
...
Then validate the expected request count (15 total calls = 3 versions × 5 tests) in Application Insights Logs.

1. Open the [Azure portal](https://portal.azure.com) and go to your lab's **Application Insights** resource.
1. Select **Logs** and run:

    ```kusto
    AppDependencies
    | where TimeGenerated > ago(30m)
    | where Name matches regex @"^v[0-9]+_"
    | summarize
        total_requests = count(),
        total_tokens = sum(tolong(Properties["response.total_tokens"])),
        prompt_tokens = sum(tolong(Properties["response.prompt_tokens"])),
        completion_tokens = sum(tolong(Properties["response.completion_tokens"]))
    ```

1. Confirm:
    - `total_requests` should be **15** for one full run of `v1`, `v2`, `v3` over five test prompts.
    - Token totals should be non-zero and align with the relative trends you observed in `check_traces.py`.
